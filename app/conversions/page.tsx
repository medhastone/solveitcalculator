...
export default function ConversionCenterPage() {
  ...
  const searchSuggestions = useMemo(() => {
    ...
    for (const pop of POPULAR_CONVERSIONS_DATA) {
      if (
        ...
        q.includes(pop.fromSymbol.toLowerCase()) ||
        q.includes(pop.toSymbol.toLowerCase())
      ) {
        results.push({
          type: 'pair',
          title: pop.name,
    ...
    for (const cat of CONVERSION_CATEGORIES) {
      ...
      for (const u of cat.units) {
        if (u.name.toLowerCase().includes(q) || u.symbol.toLowerCase().includes(q)) {
          results.push({
            type: 'unit',
            ...
            categoryId: cat.id,
            fromId: u.id,
            toId: cat.baseUnitId !== u.id ? cat.baseUnitId : cat.units.find(x => x.id !== u.id)?.id || u.id
          });
        }
      }
    }
    ...
    return results.filter(item => {
      const key = `${item.categoryId}-${item.fromId}-${item.toId}`;
  ...
  return (
    <div className={isDarkMode ? 'dark bg-[#090D16] text-slate-100 min-h-screen transition-colors duration-200' : 'bg-[#FAF8FF] text-[#131B2E] min-h-screen transition-colors duration-200'}>
            ...
            '@graph': [
              {
                ...
                name: 'Unit Converter & Conversion Tools | SolveIt Calculator',
                description: 'Universal conversion center for length, mass, temperature, area, volume, speed, pressure, energy, and hundreds of measurements.',
                inLanguage: 'en-US'
              },
              {
                '@type': 'SoftwareApplication',
                name: 'SolveIt Universal Conversion Center',
      ...
      <main className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-16">
        ...
        <section className="relative pt-6 md:pt-10 pb-4 text-center space-y-6">
          ...
          {/* Top Brand Tagline Badge & Mode Switcher */}
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            {mounted && (
              <button
                type="button"
                onClick={toggleTheme}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all shadow-sm cursor-pointer"
          ...
          <div className="max-w-2xl mx-auto relative z-30">
            <div className={`relative flex items-center rounded-2xl transition-all duration-200 shadow-lg ${
              ...
              {universalQuery ? (
                <button
                  ...
                  className="mr-3 p-1 rounded-md text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                >
                  ...
                  {searchSuggestions.length > 0 ? (
                    searchSuggestions.map((sug, idx) => (
                      <button
                        ...
                        className="w-full px-4 py-3 text-left flex items-center justify-between hover:bg-blue-50/70 dark:hover:bg-slate-800/70 transition-colors group"
                      >
                        <div className="flex items-center gap-3">
                          <span className="material-symbols-outlined text-blue-600 dark:text-blue-400 text-[20px]">
                            {sug.type === 'category' ? 'category' : 'swap_horiz'}
                          </span>
                          <div>
                        ...
                        <span className="material-symbols-outlined text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 text-[18px] transition-transform group-hover:translate-x-0.5">
                          arrow_forward
                        </span>
                      </button>
                    ))
            ...
            {/* Trust Badges Grid matching Engine RPM design */}
            <div className="pt-4 max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { icon: 'precision_manufacturing', title: 'IEEE 754 Float Math', subtitle: 'Double precision accuracy' },
                { icon: 'speed', title: '<0.02s Client Exec', subtitle: 'Zero latency instant calc' },
                { icon: 'shield', title: 'Zero Telemetry', subtitle: 'Private in-browser sandbox' },
                { icon: 'devices', title: 'Mobile PWA Ready', subtitle: 'Responsive on all viewports' }
              ].map((badge, idx) => (
                <div
                  key={idx}
                  className="p-3 rounded-xl bg-surface-container-lowest border border-outline-variant/30 shadow-xs flex items-center gap-2.5 text-left"
                >
                  <div className="w-8 h-8 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-[18px]">{badge.icon}</span>
                  </div>
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-on-surface truncate">{badge.title}</div>
                    <div className="text-[10px] text-on-surface-variant truncate">{badge.subtitle}</div>
                  </div>
                </div>
              ))}
            </div>

            {/* Quick Presets Bar matching Powertrain presets */}
            <div className="pt-2 max-w-5xl mx-auto flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
              <span className="text-xs font-label-caps uppercase text-on-surface-variant tracking-wider whitespace-nowrap shrink-0 flex items-center gap-1 font-semibold">
                <span className="material-symbols-outlined text-[15px] text-primary">tune</span>
                <span>Quick Presets:</span>
              </span>
              {[
                { label: 'Highway Speed (70 mph → km/h)', cat: 'speed', from: 'mph', to: 'kmh', val: 70 },
                { label: 'Tire Pressure (32 psi → bar)', cat: 'pressure', from: 'psi', to: 'bar', val: 32 },
                { label: 'Lug Nut Torque (100 lb·ft → N·m)', cat: 'torque', from: 'lbft', to: 'nm', val: 100 },
                { label: 'Human Mass (165 lb → kg)', cat: 'weight', from: 'lb', to: 'kg', val: 165 },
                { label: 'Room Dimension (12 ft → m)', cat: 'length', from: 'ft', to: 'm', val: 12 },
                { label: 'Baking Volume (500 mL → cups)', cat: 'volume', from: 'ml', to: 'cup', val: 500 },
                { label: 'EV Battery (85 kWh → MJ)', cat: 'energy', from: 'kwh', to: 'mj', val: 85 },
                { label: 'Engine Power (300 hp → kW)', cat: 'power', from: 'hp', to: 'kw', val: 300 }
              ].map((preset, i) => (
                <button
                  key={i}
                  type="button"
                  onClick={() => loadConversionPair(preset.cat, preset.from, preset.to, preset.val)}
                  className="whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium bg-surface-container-low hover:bg-surface-container text-on-surface-variant hover:text-primary transition-all border border-outline-variant/30 cursor-pointer shrink-0"
                >
                  {preset.label}
                </button>
              ))}

...
html_content = r'''<script type="application/ld+json">
  {
...
# Add script to the return value of page.tsx if it doesn't already have it
if "application/ld+json" not in page_content:
    # Use dangerouslySetInnerHTML
    react_script = f"<script type=\"application/ld+json\" dangerouslySetInnerHTML={{{{__html: `{html_content.split('>', 1)[1].rsplit('<', 1)[0]}`}}}} />"
    page_content = re.sub(r'(<DailyWageClient />)', r'{}\n      \1'.format(react_script), page_content)
